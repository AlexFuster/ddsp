...
def compute_loudness(audio,
  ...
  loudness = _make_vector_expected_length(loudness, expected_len, -range_db)
  return loudness
...
def compute_f0(audio, sample_rate, frame_rate, viterbi=True):
  ...
  f0_hz = _make_vector_expected_length(f0_hz, expected_len, 0)  # pad with 0
  f0_hz = f0_hz.astype(np.float32)

  # Postprocessing on f0_confidence
  f0_confidence = _make_vector_expected_length(f0_confidence, expected_len, 1)
  f0_confidence = np.nan_to_num(f0_confidence)   # Set nans to 0 in confidence
  f0_confidence = f0_confidence.astype(np.float32)
  return f0_hz, f0_confidence


def _make_vector_expected_length(vector,
                                 expected_len,
                                 pad_value=0,
                                 len_tolerance=20):
  ...
  or `hop_size`. This function corrects vectors to the expected length.

  Args:
    vector: 1D array
    expected_len: expected length of vector
    pad_value: value to pad at end
  ...
  if abs(len(vector) - expected_len) > len_tolerance:
    ...
    raise ValueError(
        ...
        "beyond tolerance of {}"
        .format(len(vector), expected_len, len_tolerance))
