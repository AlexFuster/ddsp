...
from ddsp.spectral_ops import _make_vector_expected_length
...
def _load_audio_as_array(audio_path: str,
  ...
  with tf.io.gfile.GFile(audio_path, 'rb') as f:
    ...
    audio = _make_vector_expected_length(audio, expected_len)
...
def _add_f0_estimate(ex, sample_rate, frame_rate):
  ...
  print('_add_f0_estimate f0_hz.shape', f0_hz.shape)
  ex = dict(ex)
  ex.update({
